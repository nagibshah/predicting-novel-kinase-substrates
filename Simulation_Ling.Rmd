...
model.func <- function (dt.model.full, dt.labeled, cost.n, degree.n, dt.pred) {
...
dim(dt.Akt.fulmodel)
Akt.final.fullmodel <- model.func(dt.Akt.fulmodel, dt.Akt, 1, 1, dt.Akt.fulmodel[, 2:ncol(dt.Akt.fulmodel)])
Akt.final.fullmodel[Akt.final.fullmodel$Class==1, ]
Akt.final.fullmodel[Akt.final.fullmodel$Identifier %in% dt.mTOR$V1, ]

# this is for adaptive sampling function 2 
Akt.neg <- dt.Akt.fulmodel[which(dt.Akt.fulmodel$Identifier %in% Akt.final.fullmodel[Akt.final.fullmodel$predictResult < 0.5, ]$Identifier & !dt.Akt.fulmodel$Identifier %in% dt.Akt$V1), ]
...
mTOR.final.fullmodel <- model.func(dt.mTOR.fulmodel, dt.mTOR, 1, 1, dt.mTOR.fulmodel[, 2:ncol(dt.mTOR.fulmodel)])
mTOR.final.fullmodel[mTOR.final.fullmodel$Class==1, ]
mTOR.final.fullmodel[mTOR.final.fullmodel$Identifier %in% dt.Akt$V1, ]

# this is for adaptive sampling function 2 
mTOR.neg <- dt.mTOR.fulmodel[which(dt.mTOR.fulmodel$Identifier %in% mTOR.final.fullmodel[mTOR.final.fullmodel$predictResult < 0.8, ]$Identifier & !dt.mTOR.fulmodel$Identifier %in% dt.mTOR$V1), ]
...
# function to build model 
model.adp.func <- function (dt.model.full, dt.labeled, cost.n, degree.n, dt.pred) {
  ...
  # data frame to store pred negative obs
  dt.pred.neg <- dt.model.full[dt.model.full$Class == -1, ]
  ...
  for (i in 1:1000) {
    ...
    #View(dt.model)
    #dt.model <- rbind(dt.model.full[dt.model.full$Class==1,], dt.nolabel.sample)
    dt.model <- rbind(dt.model.full[dt.model.full$Class==1,], dt.pred.neg[sample(nrow(dt.pred.neg), nrow(dt.labeled)), ])
    # build SVM classification model incl probability -- temporarily without seq.window
    #print(head(dt.model[, 2:ncol(dt.model)]))
    fit.model <- svm(Class ~ ., data=dt.model[, 2:ncol(dt.model)], kernel="radial", type="C-classification", cost=cost.n, degree=degree.n, decision.values = TRUE, probability=TRUE)
    ...
    max.rank <- max(dt.result[dt.result$Class==1, ]$prob.rank)
    #View(dt.result[order(dt.result$prob.rank), ])
    
    dt.pred.neg <- dt.model.full[which(dt.model.full$Identifier %in% dt.result[dt.result$prob.rank > max.rank, ]$Identifier), ]
...
Akt.final.adp <- model.adp.func(dt.Akt.fulmodel, dt.Akt, 1, 1, dt.Akt.fulmodel[, 2:ncol(dt.Akt.fulmodel)])
Akt.final.adp[Akt.final.adp$Class==1, ]
Akt.final.adp[Akt.final.adp$Identifier %in% dt.mTOR$V1, ]
diff.func ("predictResult", df.Akt, Akt.final.adp, "Akt Adp Model - ")

mTOR.final.adp <- model.adp.func(dt.mTOR.fulmodel, dt.mTOR, 1, 1, dt.mTOR.fulmodel[, 2:ncol(dt.mTOR.fulmodel)])
mTOR.final.adp[mTOR.final.adp$Class==1, ]
mTOR.final.adp[mTOR.final.adp$Identifier %in% dt.Akt$V1, ]
diff.func ("predictResult", df.mTOR, mTOR.final.adp, "mTOR Adp Model - ")
...
model.adp2.func <- function (dt.model.full, dt.labeled, dt.neg, cost.n, degree.n) {
...
Akt.adp2 <- model.adp2.func(dt.Akt.fulmodel, dt.Akt, Akt.neg, 1, 1)
...
mTOR.adp2 <- model.adp2.func(dt.mTOR.fulmodel, dt.mTOR, mTOR.neg, 1, 1)
...
df.mTOR[which(df.mTOR$Identifier %in% dt.Akt$V1), ]
...
# compare motif predict
# head(dt.Insulin)
dt.pred.AktMotif <- dt.Insulin[, c("Identifier", "aktMotifMatch")]
names(dt.pred.AktMotif)[colnames(dt.pred.AktMotif) == "aktMotifMatch"] <- "Motif.predict"
diff.func ("Motif.predict", df.Akt, dt.pred.AktMotif, "Akt Motif - ")

dt.pred.mTORMotif <- dt.Insulin[, c("Identifier", "mTORMotifMatch")]
names(dt.pred.mTORMotif)[colnames(dt.pred.mTORMotif) == "mTORMotifMatch"] <- "Motif.predict"
diff.func ("Motif.predict", df.mTOR, dt.pred.mTORMotif, "mTOR Motif - ")
```
...
# get predict result for simulated data 
dt.simulation.Akt <- model.func(dt.Akt.fulmodel, dt.Akt, 1, 1, dt.simulation.Akt)
dt.simulation.mTOR <- model.func(dt.mTOR.fulmodel, dt.mTOR, 1, 1, dt.simulation.mTOR)
#head(dt.simulation.Akt)

# plot given insulin data prob and simulation data prob for Akt 
par(mfrow=c(1, 2))
x <- dt.simulation.Akt[, c("predictResult")]
...
x <- Akt.final.fullmodel [, c("predictResult")]
...
print("From the plot, we can see Akt prediction results for both simulation data and give data are following the similar curve ")

# plot given insulin data prob and simulation data prob for mTOR 
par(mfrow=c(1, 2))
x <- dt.simulation.mTOR[, c("predictResult")]
...
x <- mTOR.final.fullmodel [, c("predictResult")]
...
print("From the plot, we can see mTOR prediction results for both simulation data and give data are following the similar curve ")
