...
author: "Ling Qi, Nagib Shah, Xinan Ma, Young Choi"
...
## Temproal profile fitting and average scores 
...
## expand some of the other features to allow separation at higher dimensional space 
#  AUC ^ 2, 

expandedFeatures <- NULL
featuresToExpand <- c("X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score")
expansionPool <- dt.Insulin[,featuresToExpand]

for (i in 1:ncol(expansionPool)) {
  exp2 <- sapply(expansionPool[,i],FUN = function(x) x ^ 2)
  expandedFeatures <- cbind(expandedFeatures, exp2)
  colnames(expandedFeatures)[i] <- paste(colnames(expansionPool)[i],"Exp2",sep = "")
}

expandedFeatures <- as.data.frame(expandedFeatures)

expandedFeatures$Identifier <- dt.Insulin$Identifier

dt.Insulin <- merge(dt.Insulin, expandedFeatures, by=c("Identifier"))
...
# total 32 features approx
#predictorsAkt <- c("X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score","aktMotifMatch","Kinases")
predictorsAkt <- c("X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2","aktMotifMatch","Kinases")

#predictorsmTOR <- c("X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score","mTORMotifMatch","Kinases")
predictorsmTOR <- c("X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2","mTORMotifMatch","Kinases")
...
colnames(dt.feature.selection.akt)[ncol(dt.feature.selection.akt)] <- "Class"
colnames(dt.feature.selection.mTOR)[ncol(dt.feature.selection.mTOR)] <- "Class"
...
sampleData <- function (positives, featureSpace) {
      colnames(positives)[ncol(positives)] <- "Kinases" # change col name 
      ...
      colnames(balanced)[ncol(balanced)] <- "Class" # change col name
      return(balanced)
}
...
AnalyseModels <- function (method, data) {
  ...
  pred <- predict(model$finalModel, train[,-ncol(train)])
  if (method=="lda") {
    rocModel <- roc(train[, ncol(train)], as.numeric(pred$class))  
  }
  else {
    rocModel <- roc(train[, ncol(train)], as.numeric(pred))  
  }
...
## run through some models to pick the best one for Akt
...
## run through some models to pick the best one for mTOR 
...
cat("SVM Radial yielded a the best overall result although the ROC curve indicates otherwise on the small subset.")  
<<<<<<< HEAD

cat("\n\n\nSVM Radial is understood to be a squared exponential kernel and as such categegorised as  non-parametric method. On the other hand SVM Poly kernel and linear kernels are parametric.\n\nAs such the biggest benefit with SVM Radial is that the complexity of the model can be potentially infinite and can define more complex relationship within the feature space to separate the data and thereby make classifications possible.\n\nAs such the biggest benefit with SVM Radial is that the complexity of the model can be potentially infinite and can define more complex relationship within the feature space to separate the data and thereby make classifications possible.\n\nIn contrast the SVM linear and poly are parametric and as such makes assumptions about the distributions of the data which are not always correct. Especially, since we have very little labelled positives and weak assumptions about the samples being negative (from random sample) a nonparametric method is thought to be performing better (SVM Radial).\n\nIn short, SVM Radial is the better option as it can scale well with complexity to define and separate the negatives from the positives more effectively.")

cat("\n\n\nBest Model tuning parameters - Akt")
cat("\nSVM Radial - Sigma: 1, C: 1.1")
...
>>>>>>> a52822e6efcca96cb6542569b1ec85bbb871c8a5
cat("\n\nBest Model tuning parameters - mTOR")
cat("\nSVM Radial - Sigma: 1, C: 1.1")
...
tstatSort <- function (train) {
  train.byClass <- split(train[,-ncol(train)], train$Kinases)
  ...
  names(feature.pvalues) <- colnames(train[,-ncol(train)])
  ...
  for (i in length(sortedFeatures):1) {
    ...
    test.accuracies <- c()
	  ...
	  for(i in 1:length(fold)) {
        ...
        #print(truth)
        svm.model <- svm(x=trainingData, y=cls.train[-fold[[i]]], kernel="radial", type="C-classification",cost = 1.1)
...
randomAccuracies <- function(positives, featureSpace, sortedFeatures, iterations) {
  ...
  for (i in 1:iterations) {
    ...
    accuracies <- backwardStepwise(train = balanced[,-ncol(balanced)],
                                        cls.train = balanced[,ncol(balanced)],
...
# Initial plot to see what it all looks like over two features 
## the objective here is to identify the novel Akt and mTOR substrates with only a small amount of labelled data
```{r}
plot(dt.unlabelled[, c("Avg.Fold","AUC")],col="gray", pch="?", main="The Class Imbalance Problem")
points(dt.labeled.Akt[,c("Avg.Fold","AUC")], col="red", pch=20)
points(dt.labeled.mTOR[,c("Avg.Fold","AUC")], col="blue", pch=20)
...
model.adp2.func <- function (dt.model.full, dt.labeled, dt.neg, cost.n, degree.n, dt.pred, iterations) {
...
## Create Highly probable Negative subsets

### Akt

```{r}

#dt.Akt.fulmodel <- dt.Insulin[, c("Identifier", "X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "aktMotifMatch", "fitted.score")]
dt.Akt.fulmodel <- dt.Insulin[,c("Identifier", "X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2","aktMotifMatch")]

aktNancies <- model.func(dt.Akt.fulmodel, dt.Akt, 1.1, 1, 1000, dt.Akt.fulmodel[, 2:32]) # as per best tuning param val 
...
#dt.mTOR.fulmodel <- dt.Insulin[, c("Identifier", "X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "mTORMotifMatch", "fitted.score")]

dt.mTOR.fulmodel <- dt.Insulin[,c("Identifier","X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2","mTORMotifMatch")]

mTORNancies <- model.func(dt.mTOR.fulmodel, dt.mTOR, 1.1, 1, 1000, dt.mTOR.fulmodel[, 2:32])
...
Akt.neg <- dt.Akt.fulmodel[which(dt.Akt.fulmodel$Identifier %in% 
                                   aktNancies[aktNancies$predictResult < 0.4, ]$Identifier 
...
### Visualise the results of positive unlabelled learning

#### Akt Positive Unlabelled Learning
...
plot(dt.Akt.fulmodel[, c("Avg.Fold","aktMotifMatch")],col="gray", pch=20, main="Pre Positive Learning")
points(dt.feature.selection.akt[,c("Avg.Fold","aktMotifMatch")], col="red", pch="+")

# Post learning 

plot(dt.Akt.fulmodel[, c("Avg.Fold","aktMotifMatch")],col="gray", pch=20, main="Post Positive Learning")
points(Akt.neg[,c("Avg.Fold","aktMotifMatch")], col="blue", pch="-")
points(dt.feature.selection.akt[,c("Avg.Fold","aktMotifMatch")], col="red", pch="+")

```

#### mTOR positive unlabelled learning
...
plot(dt.mTOR.fulmodel[, c("Avg.Fold","mTORMotifMatch")],col="gray", pch=20, main="Pre Positive Learning")
points(dt.feature.selection.mTOR[,c("Avg.Fold","mTORMotifMatch")], col="red", pch="+")

# Post learning 

plot(dt.mTOR.fulmodel[, c("Avg.Fold","mTORMotifMatch")],col="gray", pch=20, main="Post Positive Learning")
points(mTOR.neg[,c("Avg.Fold","mTORMotifMatch")], col="blue", pch="-")
points(dt.feature.selection.mTOR[,c("Avg.Fold","mTORMotifMatch")], col="red", pch="+")
...
## Akt final predictions 
```{r, warning=FALSE}

ensembleCorrelations <- c()
# instantiate the last probability values 
lastProbabilities <- c()
Akt.adp2 <- NULL

for(x in c(10,50,100,300,500,800,1000, 1200)) { # seq(200, 1000, by=200)
  Akt.adp2 <- model.adp2.func(dt.Akt.fulmodel, dt.Akt, Akt.neg, 1.1, 1, dt.Akt.fulmodel[, 2:32],x)
  
  if (length(lastProbabilities) > 0)
  {
    currentCor <-  cor(Akt.adp2$predictResult, lastProbabilities)
    ensembleCorrelations <- c(ensembleCorrelations, currentCor)  
  }
  lastProbabilities <- Akt.adp2$predictResult
}
...
# first iteration no cors so one less
ensembleCorrelations2 <- cbind(ensembleCorrelations, c(50,100,300,500,800,1000, 1200)) 
plot(x=ensembleCorrelations2[,2],y=ensembleCorrelations2[,1],ylab="Correlations", xlab="Ensemble Size",main="Akt Optimum Ensemble size",col="red",pch=20)
lines(ensembleCorrelations2[,2],ensembleCorrelations2[,1])

print("Correlation plot indicates optimum sampling reached at 1000 for Akt.")
```

## mTOR final predictions 
```{r}
...
for(x in c(10,50,100,300,500,800,1000, 1200)) { # seq(200, 1000, by=200)
  mTOR.adp2 <- model.adp2.func(dt.mTOR.fulmodel, dt.mTOR, mTOR.neg, 1.1, 1, dt.mTOR.fulmodel[, 2:32],x)
  
  if (length(lastProbabilities) > 0)
  {
    currentCor <-  cor(mTOR.adp2$predictResult, lastProbabilities)
    ensembleCorrelations <- c(ensembleCorrelations, currentCor)  
  }
  lastProbabilities <- mTOR.adp2$predictResult
}
...
# plot the correlations 
ensembleCorrelations2 <- cbind(ensembleCorrelations, c(50,100,300,500,800,1000,1200))
plot(x=ensembleCorrelations2[,2],y=ensembleCorrelations2[,1],ylab="Correlations", xlab="Ensemble Size",main="mTOR Optimum Ensemble size",col="red",pch=20)
lines(ensembleCorrelations2[,2],ensembleCorrelations2[,1])

print("Correlation plot indicates optimum sampling reached at 800 for mTOR.")
...
#feat.list <- dt.Insulin[, c("X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2", "aktMotifMatch", "mTORMotifMatch")]

feat.list <- dt.Insulin[, c("X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "aktMotifMatch", "mTORMotifMatch", "fitted.score")]
...
dt.simulation.func <- function(dt){
  ...
  fullset <- NULL
  Identifier <- seq.int(1, 200000, 1)
  fullset <- cbind(fullset, Identifier)
  
  for (i in 1:ncol(dt)) {
    set.seed(123)
    x <- rnorm(n=200000, m=dt[c("Mean"), i], sd=dt[c("sd"), i])
...
#colnames(dt.sim) <- c("Identifier", "X15s", "X15sExp2","X30s","X30sExp2", "X1m","X1mExp2", "X2m","X2mExp2", "X5m","X5mExp2", "X10m","X10mExp2", "X20m","X20mExp2", "X60m","X60mExp2", "Avg.Fold","Avg.FoldExp2", "AUC","AUCExp2", "Ins.1","Ins.1Exp2", "LY","LYExp2", "Ins.2","Ins.2Exp2", "MK","MKExp2", "fitted.score","fitted.scoreExp2", "aktMotifMatch", "mTORMotifMatch")

colnames(dt.sim) <- c("Identifier", "X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "aktMotifMatch", "mTORMotifMatch", "fitted.score")

## generate the exp2 data 

expandedFeatures <- NULL
featuresToExpand <- c("X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score")
expansionPool <- dt.sim[,featuresToExpand]
...
expandedFeatures$Identifier <- dt.sim$Identifier

dt.sim <- merge(dt.sim, expandedFeatures, by=c("Identifier"))
...
dt.simulation.Akt <- dt.sim[sample(1:nrow(dt.sim), size = nrow(dt.Insulin)), -which(names(dt.sim) == "mTORMotifMatch")]
dt.simulation.mTOR <- dt.sim[sample(1:nrow(dt.sim), size = nrow(dt.Insulin)), -which(names(dt.sim) == "aktMotifMatch")]

# final prediction for simulated data 
Akt.sim.adp2 <- model.adp2.func(dt.Akt.fulmodel, dt.Akt, Akt.neg, 1.1, 1, dt.simulation.Akt,1000)

mTOR.sim.adp2 <- model.adp2.func(dt.mTOR.fulmodel, dt.mTOR, mTOR.neg, 1.1, 1, dt.simulation.mTOR,1000)
...
h<-hist(x, breaks=30, col="red", xlab="Predict Result", main="Akt Actual Data")
xfit<-seq(min(x),max(x),length=40)
yfit<-dnorm(xfit,mean=mean(x),sd=sd(x))
yfit <- yfit*diff(h$mids[1:2])*length(x)
lines(xfit, yfit, col="blue", lwd=2)
...
h<-hist(x, breaks=30, col="red", xlab="Predict Result", main="mTOR Actual Data")
xfit<-seq(min(x),max(x),length=40)
yfit<-dnorm(xfit,mean=mean(x),sd=sd(x))
yfit <- yfit*diff(h$mids[1:2])*length(x)
lines(xfit, yfit, col="blue", lwd=2)

cat("\nFrom the plot, we can see mTOR prediction results for both simulation data and actual data are following the similar distribution curve ")
