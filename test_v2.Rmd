...
# train the SVM Polynomial model
set.seed(7)
...
control <- trainControl(method="repeatedcv", number=10, repeats=3)
# train the model
model <- train(Kinases~., data=dt.feature.selection, method="svmPoly", preProcess="scale", trControl=control)
...
for (i in 1:200) {
  ...
  # due to small size of labeled data, we bootstrap to make labeled data size 20 times, and randomly selected the same size of unlabled data for training, make total observation Akt 20*22*2 = 880, mTOR 20*26*2 = 1040 
  dt.nolabel.sample <- dt.model.full[dt.model.full$Class==-1,][sample(x=1:nrow(dt.model.full[dt.model.full$Class==-1, ]), size = nrow(dt.labeled)*20, replace = TRUE),]
  dt.labeled.sample <- dt.model.full[dt.model.full$Class==1,]
  #dt.labeled.sample[dt.labeled.sample$Class==1, ]
  
  dt.model <- rbind(dt.labeled.sample[sample(x=1:nrow(dt.labeled.sample), size=nrow(dt.labeled)*20, replace=TRUE), ], dt.nolabel.sample)
  #dim(dt.model)
  #head(dt.model)
  #View(dt.model)
...
dt.Akt.fulmodel <- dt.Insulin[, c("Identifier", "X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score")]
dim(dt.Akt.fulmodel)
...
dt.mTOR.fulmodel <- dt.Insulin[, c("Identifier", "X15s", "X30s", "X1m", "X2m", "X5m", "X10m", "X20m", "X60m", "Avg.Fold", "AUC", "Ins.1", "LY", "Ins.2", "MK", "fitted.score")]
...
df.Akt.motif <- df.Akt.full
names(df.Akt.full)[colnames(df.Akt.full)=="Full.model.predict"] <- "predictResult" 
names(df.Akt.motif)[colnames(df.Akt.motif)=="Motif.predict"] <- "predictResult" 
...
df.mTOR.motif <- df.mTOR.full
names(df.mTOR.full)[colnames(df.mTOR.full)=="Full.model.predict"] <- "predictResult" 
names(df.mTOR.motif)[colnames(df.mTOR.motif)=="Motif.predict"] <- "predictResult" 

diff.func ("predictResult", df.Akt.full, Akt.final.fullmodel, "Akt Full Model - ")
diff.func ("predictResult", df.Akt.motif, Akt.final.motif, "Akt Motif Model - ")
diff.func ("predictResult", df.mTOR.full, mTOR.final.fullmodel, "mTOR Full Model - ")
diff.func ("predictResult", df.mTOR.motif, mTOR.final.motif, "mTOR Motif Model - ")
