...
# SVM Radial yielded a the best overall result although the ROC curve indicates otherwise on the small subset 
...
# Semi adaptive function to learn negative data - with LOOCV + dynamic tuning parameters for each sample
letThereBeLightv2 <- function (dt.model.full, dt.labeled, cost.n, degree.n) {
  ...
  for (i in 1:100) {
    ...
    dt.model <- rbind(dt.model.full[dt.model.full$Class==1,], 
                      dt.pred.neg[sample(nrow(dt.pred.neg), nrow(dt.labeled)),])
    
    dt.model$Class <- as.character(dt.model$Class)
    dt.model[which(dt.model$Class == "1"),]$Class <- "POSITIVE"
    dt.model[which(dt.model$Class == "-1"),]$Class <- "NEGATIVE"
    dt.model$Class <- as.factor(dt.model$Class)
    
    # train run validation and find the best c value for the dataset in question 
    model <- train(Class~., data=dt.model[, 2:ncol(dt.model)], 
                   method="svmRadial", 
                   trControl=controlLOOCV,tuneGrid=tuneGrid)
    
    # now predict on the full unlabelled set 
    pred <- predict(model$finalModel, dt.model.full[, 2:last.col],type="prob")
  
    currentProbabilities <- pred[,2] # attr(pred, "probabilities")[,1]
    ...
    dt.result <- transform(dt.result, 
                           prob.rank = ave(dt.result[, ncol(dt.result)], 
                                           na.last = TRUE, 
                                           FUN = function(x) rank(-x, ties.method = "last")))
    
    max.rank <- max(dt.result[dt.result$Class==1, ]$prob.rank)
    
    filtered <- which(dt.model.full$Identifier %in% dt.result[dt.result$prob.rank > max.rank, ]$Identifier)
    dt.pred.neg <- dt.model.full[filtered, ]
...
# Semi adaptive function to learn negative data
model.adp.func <- function (dt.model.full, dt.labeled, cost.n, degree.n, iterations) {
  ...
  for (i in 1:iterations) {
    ...
    dt.model <- rbind(dt.model.full[dt.model.full$Class==1,], 
                      dt.pred.neg[sample(nrow(dt.pred.neg), 
                                         nrow(dt.labeled)), ])
    # build SVM classification model incl probability
    fit.model <- svm(Class ~ ., data=dt.model[, 2:ncol(dt.model)], 
    ...
    dt.pred.neg <- dt.model.full[which(dt.model.full$Identifier 
                                       %in% 
                                         dt.result[dt.result$prob.rank > max.rank, ]$Identifier), ]
...
# basic function to random sample including LOOCV and dynamic tuning per sample
letThereBeLight <- function (data,cost.n, degree.n) {

  # prep the full unlabelled set
  featureSpace <- colnames(data)
  unlabelled.all <- dt.Insulin[,featureSpace]
  # data frame to store pred result
  dt.result <- dt.Insulin[, c("Identifier","Kinases")]
  colnames(unlabelled.all)[17] <- "Class"
  colnames(dt.result)[2] <- "Class"
  
  # covert the classes to int
  dt.result[which(dt.result$Class == 'Akt')]$Class <- "1"
  dt.result[which(dt.result$Class == 'mTOR')]$Class <- "-1"
...
aktNancies <- model.adp.func(dt.Akt.fulmodel, dt.Akt, 1.1, 1, 1000) # as per best tuning param val 
...
mTORNancies <- model.adp.func(dt.mTOR.fulmodel, dt.mTOR, 1.1, 1, 1000)
...
# negative subset for ensemble step
...
Akt.adp2 <- model.adp2.func(dt.Akt.fulmodel, dt.Akt, Akt.neg, 1.1, 1)
Akt.adp2[Akt.adp2$Class==1, ]
Akt.adp2[Akt.adp2$Identifier %in% dt.mTOR$V1, ]


mTOR.adp2 <- model.adp2.func(dt.mTOR.fulmodel, dt.mTOR, mTOR.neg, 1.1, 1)
